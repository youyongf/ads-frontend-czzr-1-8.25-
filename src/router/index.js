  ...
  
  {
    ...
    children: [
      ...
      },
      {
        ...
        path: '/onLineQtimeBaskVol_warning',
        name: '任务时长统计',
        component: () => import('../pages/report/onLineQtimeBaskVol_warning.vue'),
      }
    ]
  },
...
async function initLocaleLanguage(localeLang) {
    console.log(localeLang,"设置加载语言")
    const msgs = await import(/* webpackChunkName: "lang-[request]" */ `@/language/${localeLang}.js`);
...
router.beforeEach(async (to, from, next) => {
  ...
  if(from.name === null) {
    let localeLangCache = localLang;
    if(localLang == "null" || !localLang) localeLangCache = "ch"
    await initLocaleLanguage(localeLangCache);
  }
