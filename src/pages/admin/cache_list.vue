<template>
  <div>
    ...
    <a-modal
        ...
        @cancel="handleCancel"
    >
      <station_detail :data="record" :typeName="'edit'" :processes="processes" :steps="steps" :parks="parks" :groups="groups" :nextDecidePoints="nextDecidePoints"
       :nextCacheStations="nextCacheStations" :stations="stationsData" :nextStationsCN="nextStationsCN" :alldata="alldata"/>
    </a-modal>
    <!-- 新增机台弹窗 -->
    <a-modal
        :title="$tcache('admin.modalTile')"
    ...
    >
      <station_detail :data="newStationInfo" :typeName="'add'" :processes="processes" :steps="steps" :parks="parks"
       :nextCacheStations="nextCacheStations" :stations="alldata" :alldata="[]"/>
    </a-modal>
  </div>
</template>
