...
<!--      <polyline v-if="route.pointstr" :points="route.pointstr" :style="route.style" />-->
    ...
    <a-drawer :title="$tcache('admin.drawerTitle01')"
      ...
      <a-row type="flex" justify="start" :gutter="{ xs: 8, sm: 16, md: 24, lg: 32 }" class="bgcolor">
        <a-col :span="24">
          {{$tcache('admin.type')}}：<a-select v-model="ca.type" :options="caType" style="width: 200px;"></a-select>
        </a-col>
...
export default {
  ...
  data(){
    return{
      ...
      point:{},
      caType:[
        {value:1,label:this.$tcache('admin.type01')},
        {value:2,label:this.$tcache('admin.type02')},
        {value:3,label:this.$tcache('admin.type03')},
        {value:4,label:this.$tcache('admin.type04')},
        {value:6,label:this.$tcache('admin.type05')}
      ]
    }
  },
  filters:{
  ...
  methods:{
    init(){
      this.axios.all([this.queryPoints(), this.queryStations(), this.queryCtrlAreas()])
          ...
          if(res1.status == 200){
            res1.data=res1.data.filter(p=>p.x!==999999 && p.y!==999999)
