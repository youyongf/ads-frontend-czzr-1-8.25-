  ...
  methods:{
    ...
    queryStations(){
      this.axios.get(`station/all`).then(res=>{
        ...
        //   return {value:s.doPoint,label:s.name}
        // })
        // let arr = res.data.map(s=>{
        //   return {value:s.doPoint,label:s.name}
        // })
        // this.$set(this.checkboxs,"nextOutBasketStation",arr)
      })
    },
    queryData(){
      let that = this
    ...
    recordEdit(index,record){
      ...
      this.type='edit'
      record.buffers=record.buffers?record.buffers.split(',').map(n=>Number(n)):undefined
      record.nextOutBasketStation = record.nextOutBasketStation?.split(',')?.map(d=>Number(d))
      console.log(record.nextOutBasketStation)
