<template>
  <a-form-model  layout="inline" :form="data">
    <div class="flex">
      ...
      <a-form-model-item class="flex_item Width_350" label="汇入点">
        <a-input type="number" class="changeWidth" v-model.number="data.plungePoint" placeholder="汇入点" />
      </a-form-model-item>
      ...
      <a-form-model-item  class="flex_item Width_350" :label="$tcache('admin.stationDet15')">
        ...
        <a-select v-model="data.alternateGroups"  class="changeWidth" :allow-clear="true">
          <a-select-option class="" v-for="item in groups" :key="item.value" :value="`${item.value}`">{{item.label}}</a-select-option>
        </a-select>
      </a-form-model-item>
      <a-form-model-item  class="flex_item Width_350">
      ...
      <a-form-model-item  class="flex_item Width_350">
        ...
        <a-input  class="changeWidth" type="number" v-model.number="data.adsPort" :placeholder="$tcache('admin.stationDet17')" />
      </a-form-model-item>
      <a-form-model-item  class="flex_item Width_350">
...
export default {
  ...
  watch:{
    ...
    'data.relPids'(newV){
      const regex = /[\uFF0C]/; // 正则表达式匹配中文逗号的Unicode编码
      if(regex.test(newV)){
        this.data.relPids = this.data.relPids.replace(regex,",");
      }
    },
  },
  mounted(){
