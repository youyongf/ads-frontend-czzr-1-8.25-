<template>
  <div>
    ...
    <div class="colorBox">
      ...
      <div>
        根据区域显示机台：<a-select v-model="area" class="select" placeholder="区域" :allow-clear="true" :options="areas" />(此选项不会关联其他修改项)
        <a-popconfirm
    ...
    >
      <station_detail :data="record" :typeName="'edit'"  :parks="parks"
        :stations="stationsData" :alldata="stations" :groups="groups" :steps="steps" :num="num"/>
    </a-modal>
    <a-modal
    ...
    >
      <publicDetail :data="updates" type="edit" :columns="columns2" :checkbox="checkbox" :checkboxs="checkboxs" :disabled="disabled" :isNum="isNum"
       :radio="radio" :radios="radios" :hidden="hidden" :groups="groups" :num="num"/>
    </a-modal>
    <a-row>
      <a-col span="11">
...
import publicDetail from "@/components/stationGroup_detail";
...
export default {
  ...
  data(){
    return{
      ...
      checkboxs:{},
      isNum:['area','limits'],
      radio:['chargerLimits','alternateGroups'],
  ...
  mounted() {
    ...
    this.init()
    vueApp=this
    this.updates={id:'',nextStations1:undefined,nextStations2:undefined,parks:undefined,nextCacheStations:undefined,area:undefined,limits:undefined,chargerLimits:undefined,alternateGroups:undefined}
  },
  filters:{
    fmtStep:val=>{
  ...
  methods:{
    ...
    updataNextStations(){
      ...
      this.axios.post(`station/updateStations`,obj).then(res=>{
        ...
        this.updates={id:obj.id,nextStations1:undefined,nextStations2:undefined,parks:undefined,nextCacheStations:undefined,area:undefined,limits:undefined,chargerLimits:undefined,alternateGroups:undefined}
    ...
    recordEdit(s,k,i){
      ...
      if((parks??'')!=''&&typeof parks!=='object'){
        s.parks=parks.split(',').map(n=>Number(n))
      }
      this.$set(s,"nextStations1",[]);
      this.$set(s,"nextStations2",[]);
      if(s.nextStations?.includes(';')){
        let nextStations = s.nextStations.split(';');
        s.nextStations1 = nextStations[0].split(',').map(n=>Number(n))
        if(nextStations[1].includes(';')){
          s.nextStations2 = nextStations[1].split(';')[0].split(',').map(n=>Number(n))
        }else {
          s.nextStations2 = nextStations[1].split(',').map(n=>Number(n))
        }
      }else if(s.nextStations){
        s.nextStations1 = s.nextStations.split(',').map(n=>Number(n))
      }
    ...
    // TOTO 中文判断
    saveStation(){
      // this.dealt(this.record.nextStations1,'nextStations1')
      // this.dealt(this.record.nextStations2,'nextStations2')
      console.log(this.record.nextStations.split(';'))
      ...
      if(this.record.nextStations1.length>0){
        let nextStations = null;
        if(this.record.nextStations?.includes(';')){
          let ns = this.record.nextStations.split(';');
          if(ns.length>2){
            nextStations = this.record.nextStations.split(';');
            nextStations.splice(0,2)
            nextStations = nextStations.join(';')
          }
          console.log(nextStations)
        }
        if(this.record.nextStations2.length>0){
          this.record.nextStations = this.record.nextStations1.join(',') + ';' + this.record.nextStations2.join(',');
          if(nextStations)this.record.nextStations+=';'+nextStations;
          console.log(this.record.nextStations)
        }else{
          this.record.nextStations = this.record.nextStations1.join(',');
          if(nextStations)this.record.nextStations+=';'+nextStations;
          console.log(this.record.nextStations)
        }
      }
    ...
    queryGroup(){
      this.axios.get(`stationGroup/list`).then(res=>{
        if(res.status == 200&&res.data){
          ...
          this.radios['alternateGroups'] = this.groups
        }
      }).catch(err=>{this.$message.error(err)})
    },
    showWarn(station){ 
    ...
    stepChange(){
      ...
      let nextStep = this.stations.find(s=>s.step==this.step)?.nextStep
      console.log(this.stations.filter(s=>s.step==nextStep))
      this.checkboxs['nextStations1']=this.stations.filter(s=>s.step==nextStep).map(d=>{
        return {value:d.doPoint,label:this.changeGroup(d.group)+'-'+d.name}
      })
      this.checkboxs['nextStations2']=this.checkboxs['nextStations1']
      this.checkboxs['stations']=this.stations.filter(s=>s.step==nextStep).map(d=>{
        return {value:d.group,label:this.changeGroup(d.group)+'-'+d.name,doPoint:d.doPoint}
      })
      console.log(this.checkboxs['stations'])
    },
    save(){
    ...
    clearChecked(){
      ...
      let obj={id:this.checkedValues.join(','),nextStations:null}
      this.axios.post(`station/updateStations`,obj).then((res)=>{
        if(res.data.success){
          Object.keys(this.kvData).forEach(step=>{
            this.checkedValues.forEach(stationId=>{
              let index = this.kvData[step].findIndex(station=>stationId==station.id);
              if(index>=0){
                let obj = this.kvData[step][index];
                obj.nextStations = null;
                this.$set(this.kvData[step],index,obj)
              }
            })
          })
          this.$message.success(res.data.message)
