<template>
  <div>
    ...
    <div v-if="isAdmin|editable" @click="()=>{visibleDrawer=true;visibleDrawer_point=false}" class="icon" :title="$tcache('admin.stationMap01')"><Icon class="fz" type="ios-cog-outline" /></div>
    ...
    init() {
      this.axios.all([this.queryPoints(), this.queryStations(),this.queryPointType()])
        .then(this.axios.spread((res1, res2, res3) => {
          if (res1.status == 200) {
            res1.data = res1.data.filter(d=>d.x!==999999 && d.y!==999999)
