<template>
  <div style="background-color: #ececec; padding:0 20px;user-select:none;">
    <div style="display:flex;align-items: center;flex-wrap: wrap;">
      ...
      <a-button v-show="select=='card'" @click="stationType=-stationType" :style="{boxShadow: stationType>0?'1px 1px 5px 0 #676262 inset':'none',marginRight:'5px'}"><span class="fontSty">{{$tcache('common.cacheStation')}}</span></a-button>
      ...
      <a-radio-group v-show="select=='card'" v-model="radioValue">
        <div class="colorBox">
          <!-- <span>外框颜色标注：</span> -->
          <div class="red"><a-radio value="red">机台待料</a-radio></div>
          <div class="blue"><a-radio value="blue">机台待接料</a-radio></div>
          <div class="purple"><a-radio value="purple">机台待料/待接料</a-radio></div>
        </div>
      </a-radio-group>
    </div>
    <div v-show="select=='card'">
      ...
      <a-row :gutter="0" type="flex">
        <a-card v-for="item in listdata" :key="'s'+item.id" :title="item.name" :bordered="false" class="card" @click="handleClickStation" v-show="item.stationType>stationType && (item.boxShadow?.includes(radioValue)||!radioValue)"
  ...
  methods:{
    ...
    queryStationInfo(){
      return this.axios.get(`cache/stationinfo/list`).then(res=>{
        let data=res.data
        if(Array.isArray(data)){
          let arr = this.allstations;
          data.forEach(s=>{
            let index=arr.findIndex(a=>a.id==s.id)
            if(index!==-1){
              arr[index].offline=true
              arr[index].ts=s.ts
              arr[index].inState=s.inState
              arr[index].outState=s.outState
              arr[index].inNum=s.inNum
              arr[index].outNum=s.outNum
              arr[index].stringByControlState=s.stringByControlState
              arr[index].controlState=s.controlState
              if(s.cacheLayerInfos){
                arr[index].cacheLayerInfos=s.cacheLayerInfos
              }
              arr[index].boxShadow = "0 0 10px 0 inset"
              if(arr[index].inRail && arr[index].outRail && arr[index].inNum===0 && arr[index].outNum===10){
                arr[index].boxShadow = "0 0 10px 0 inset purple"
              }else if(arr[index].inRail && arr[index].inNum===0){
                arr[index].boxShadow = "0 0 10px 0 inset red"
              }else if(arr[index].outRail && arr[index].outNum===10){
                arr[index].boxShadow = "0 0 10px 0 inset blue"
              }
            }
          })
          this.allstations = arr;
