...
export default {
  ...
  data(){
    return{
      ...
      checkbox:false,
      uneditable:[
        '/monitor_station','/monitor_warn_journal','/monitor_offlinepointmap','/onLineQtimeBaskVol_warning',
        ...
        '/newPassword','/admin_pointmap','/admin_camap','/admin_cachelayergroup','/systemLog'
      ],//不可编辑的页面地址
    }
  },
  computed: {
      ...
      .then(res=>{
        ...
        res.data.forEach(d=>{
          ...
          roleIds.forEach(item=>{
            if(d.id==item){
              roleArr.push(item+'_'+d.roleName)
            }
          })
        })
        console.log(roleArr)
        this.usedgroups=roleArr.map(s=>{return { label: s.split('_')[1], value: s.split('_')[0], onChange:this.handleGroupItemChange }})
      }).catch(err=>{
