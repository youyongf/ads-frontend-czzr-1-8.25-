    ...
    <a-layout-sider ref="instruction_panel" style="background: #4b4b4bc2;" v-model="collapsed" :trigger="null" collapsible :collapsedWidth="60" :width="sliderWidth" :style="{position: 'fixed', zIndex: 1, width: '100%'}">
      ...
      <a-row :hidden="collapsed">
        <a-space direction="vertical">
          ...
          <span v-if="isAdmin||editable">{{$tcache('map.realTimeAGV')}}：<a-switch v-model="showagv" :checked-children="$tcache('map.open')" :un-checked-children="$tcache('map.close')" @change="onChangeShowAgv"/></span>
          ...
          <span v-show="isAdmin||editable">{{$tcache('map.trafficArea')}}：<a-switch :checked-children="$tcache('map.open')" :un-checked-children="$tcache('map.close')" @change="onChangeShowCa"/></span>
          ...
          <span v-show="isAdmin||editable">{{$tcache('map.machineStatus')}}2：<a-switch  :checked="showStationState2" :checked-children="$tcache('map.display')" :un-checked-children="$tcache('map.hide')" @change="onChangeShowStations2"/></span>
          <div v-show="isAdmin||editable" class="colormsg">(<div class="c-4"></div>：{{$tcache('map.timeout')}}<div class="c-5"></div>：{{$tcache('map.refueling')}})</div>
          <div v-show="isAdmin||editable">{{$tcache('map.pointID')}}：<a-input ref="position_point" v-model="pointid" style="width: 25%;height:10%" @focus="translatey()"></a-input><a-button :disabled="queryPointBtn" style="height: 27px;border:1px solid #40a9ff;" @click="queryPoint">{{$tcache('common.query')}}</a-button></div>
          <div v-show="isAdmin||editable">{{$tcache('map.routeID')}}：<a-input v-model="routeid" style="width: 25%;height:10%" @focus="translatey()"></a-input><a-button style="height: 27px;border:1px solid #40a9ff;" @click="queryRoute">{{$tcache('common.query')}}</a-button></div>
          <div v-show="isAdmin||editable">{{$tcache('map.carID')}}：<a-input v-model="Agv_ID" style="width: 25%;height:10%" @focus="translatey()"></a-input><a-button :disabled="queryAgvBtn" style="height: 27px;border:1px solid #40a9ff;" @click="queryAgv">{{$tcache('common.query')}}</a-button></div>
        </a-space>
      </a-row>

      <a-row :hidden="collapsed">
        <a-radio-group v-model="routefilter" button-style="solid" @change="changeRouteFilter"  v-if="isAdmin||editable">
      ...
      <div class="routes" v-if="isAdmin||editable">
    ...
    <div class="map-wrapper" :class="{'map-wrapper-mobile': clientType !== 'pc'}" :style="{marginTop: '64px'}">
      <div class="map-content" :style="{width: map.width + 'px', height: map.height + 'px'}">
        <svg xmlns="http://www.w3.org/2000/svg" :width="map.width" :height="map.height" id="map" ref="map" style="background:url(img/map.svg) no-repeat">
          ...
          <polyline v-if="route.pointstr&&isAdmin||editable" :points="route.pointstr" :style="route.style" />
          <polyline v-show="isAdmin||editable" v-for="r in routes" :key="'r'+r.id"  :points="r.pointstr" :style="r.style" >
  ...
  methods:{
    ...
    dataDeal(data){
      ...
      if(d.state||d.state==0){
        ...
        if(bol){
          let state = d.state;
          if(Array.isArray(this.toolDataCache[state])){
            this.toolDataCache[state].push(d.id)
          }else{
            this.toolDataCache[state] = []
            this.toolDataCache[state].push(d.id)
          }
        }
      }
      ...
      this.axios.get(`cache/agvinfo/list`).then(res=>{
        res.data.forEach((item)=>{
          if(this.agvmap[item.id]){
            if(this.agvmap[item.id].to!=item.to || !this.agvmap[item.id].to){
              this.agvmap[item.id].to = item.to;
            }
          }
        })
      })
