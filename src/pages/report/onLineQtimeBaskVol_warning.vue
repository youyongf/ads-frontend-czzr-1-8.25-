  ...
  <div>
    ...
    <div class="refreshTime">
      ...
      <a-input-search
        ...
        style="max-width: 200px; margin-bottom: 5px"
      />
      <a-button class="warning" type="danger" ghost @click="closeWarning">警报解除</a-button>
    </div>
    <a-row>
      <a-table
        :row-class-name="
          (record) => (record.warning ? null:'table-striped' )
        "
        :dataSource="data"
        :columns="columns"
        rowKey="item"
      ...
      <div slot="operate" slot-scope="text,record">
        <a-button :disabled="record.warning" type="danger" ghost>{{record.warning?'正常':'警报'}}</a-button>
      </div>
      </a-table>
    </a-row>
...
const tableData = require("../../data/columns_onLineQitemBaskVol_warning.json");
export default {
  data() {
    return {
      data: [],
  ...
  beforeDestroy() {
    ...
    if(this.timerWarning)clearTimeout(this.timerWarning)
  ...
  methods: {
    ...
    closeWarning(){
      ...
      Object.keys(this.isWarning).forEach(k=>{
        this.isWarning[k] = false
      })
